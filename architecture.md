...
└── defect-kb/                        # CLI 工具包
    ...
    ├── cli.py                        # 命令实现：init/govern/index/search/browse/stats/report
    ├── config.py                     # 配置加载与路径解析
    ...
    └── prompts/                      # LLM Prompt 模板
        ├── standardize.txt           # 原始文本 → Experience Card
...
```
项目根目录/
├── defect-kb.yaml                    # 项目配置（init 生成）
└── defect-kb-data/                   # 数据目录（不提交 git）
    ├── .gitignore                   # 自动生成，忽略 cards/events/chroma/reports/.venv
    ├── .venv/                       # Python 虚拟环境（bootstrap.py 自动创建）
    ├── cards.jsonl                   # 所有 Experience Card 原始数据（含 QualityScore）
    ├── events.jsonl                  # 操作事件日志（govern/search/index/search_outcome/fix_session/confidence_change）
    ├── sync-state.json              # Issue Tracker 增量同步时间戳（v2.2）
    ├── chroma_db/                    # ChromaDB 向量索引
    └── reports/                      # report 命令生成的 Markdown 质量报告
        └── report-YYYYMMDD.md
```
...
| 命令 | 功能 | 关键参数 |
|------|------|---------|
| `init` | 初始化项目配置（自动扫描项目名/repo/平台/模块/Skill/规则文件 + Markdown 预览） | `--template`、`--install-skills`、`--force`、`--no-preview`、`--confirm` |
| `govern` | 原始文本/JSON → Experience Card | `--json`（默认）、`--quality-json`（默认）、`--platform`、`--force`、`--output-format`；高级：`--input`（需 Key）、`--auto-retry`（需 Key）、`--skip-quality` |
| `index` | 未索引 Card → ChromaDB 向量索引 | `--embedding-provider {openai\|local}`、`--output-format` |
| `search` | 检索知识库（语义/混合/精排） | `--query`、`--platform`、`--top-k`、`--min-similarity`、`--hybrid`、`--rerank`、`--embedding-provider`、`--output-format {text\|json\|compact}` |
| `browse` | 按 ID 查看完整 Card（含质量评分详情） | `--id`、`--output-format` |
| `stats` | 质量统计与分布分析 | `--output-format` |
